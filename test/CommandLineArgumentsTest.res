...
Test.describe("CommandLineArguments", () => {
  ...
  Test.test("parses the template name from the --template flag for res-x", () => {
    switch CommandLineArguments.parse(list{"my-app", "--template", "res-x"}) {
    | Ok(commandLineArguments) =>
      commandLineArguments->assertCommandLineArguments(
        ~projectName=Some("my-app"),
        ~templateName=Some(Templates.resXTemplateName),
      )
    | Error(message) => Assert.fail(message)
    }
  })
  ...
  Test.test("rejects a missing template value", () => {
    assertParseError(
      ...
      ~message="Missing value for --template. Supported options: --template <vite|nextjs|xote|xote-ssr|res-x|basic> or -t <vite|nextjs|xote|xote-ssr|res-x|basic>.",
    )
  })
  ...
  Test.test("rejects unknown options", () => {
    assertParseError(
      ...
      ~message="Unknown option \"--yes\". Supported options: --template <vite|nextjs|xote|xote-ssr|res-x|basic> or -t <vite|nextjs|xote|xote-ssr|res-x|basic>.",
    )
  })
  ...
  Test.test("rejects unknown templates", () => {
    assertParseError(
      ...
      ~message="Unknown template \"unknown\". Available templates: vite, nextjs, xote, xote-ssr, res-x, basic.",
    )
  })
  ...
  Test.test("rejects positional templates", () => {
    assertParseError(
      ...
      ~message="Unexpected argument \"vite\". Supported options: --template <vite|nextjs|xote|xote-ssr|res-x|basic> or -t <vite|nextjs|xote|xote-ssr|res-x|basic>.",
    )
  })
  ...
  Test.test("rejects additional positional arguments after the template", () => {
    assertParseError(
      ...
      ~message="Unexpected argument \"extra\". Supported options: --template <vite|nextjs|xote|xote-ssr|res-x|basic> or -t <vite|nextjs|xote|xote-ssr|res-x|basic>.",
    )
  })
})
